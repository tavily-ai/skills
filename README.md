...
| **[tavily-enrich](skills/tavily-enrich/explanation.md)** | Fill in missing fields (CEO, funding, contact info, etc.) on a list of companies, people, or products from the web. |
| **[tavily-findall](skills/tavily-findall/explanation.md)** | Build a structured, deduplicated list of entities matching a description, instead of a pile of search results. |
| **[tavily-monitor](skills/tavily-monitor/explanation.md)** | Watch a page or site on a recurring schedule and get notified only when something changes. |
| **[tavily-runs](skills/tavily-runs/explanation.md)** | Look up a past research run by topic instead of by its request ID. |
| **[migrate-to-tavily](skills/migrate-to-tavily/explanation.md)** | Migrate an existing Exa, Firecrawl, Perplexity, or Parallel integration over to Tavily. |
